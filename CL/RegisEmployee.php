<?php
    ...
    $sql_employee = "INSERT INTO employee (Name, Sername, Gender, Age, Birthday, 
    ...
    if (mysqli_query($conn, $sql_employee)) {
        $login_sql_employee = "INSERT INTO login_employee (ID, Password, Email) VALUES ('$id', '$password', '$email')";
        if (mysqli_query($conn, $login_sql_employee)) {
            echo "บันทึกข้อมูลเรียบร้อยแล้ว";
        } else {
            echo "เกิดข้อผิดพลาดในการบันทึกข้อมูล login employee: " . mysqli_error($conn);
        }
    } else {
        echo "เกิดข้อผิดพลาดในการบันทึกข้อมูล employee: " . mysqli_error($conn);
    }
